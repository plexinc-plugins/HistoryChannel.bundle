...
def Episodes(show_title, episode_url, clip_url, show_thumb):
    json_data = JSON.ObjectFromURL(episode_url)
    
    if json_data['totalNumber'] < 1:
    ...
    for item in json_data['Items']:
        if not (item['isBehindWall'] == 'false'):
            continue
...
###################################################################################################
@route(PREFIX + '/createepisodeclipobject', duration=int, index=int, season=int, include_container=bool)
